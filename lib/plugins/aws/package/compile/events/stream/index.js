      ...
      if (functionObj.events) {
        ...
        functionObj.events.forEach(event => {
          if (event.stream) {
            ...
            if (event.stream.destinationConfig) {
              if (
                event.stream.destinationConfig.onFailure &&
                event.stream.destinationConfig.onFailure.destination
              ) {
                let OnFailureDestinationArn;

                if (typeof event.stream.destinationConfig.onFailure.destination === 'object') {
                  if (!event.stream.destinationConfig.onFailure.destination.arn) {
                    throw new this.serverless.classes.Error(
                      this.resolveInvalidDestinationPropertyErrorMessage(functionName, 'arn')
                    );
                  }
                  if (
                    typeof event.stream.destinationConfig.onFailure.destination.arn !== 'string'
                  ) {
                    if (!event.stream.destinationConfig.onFailure.destination.type) {
                      const errorMessage = [
                    ...
                    if (
                      !this.isValidStackImport(
                        event.stream.destinationConfig.onFailure.destination.arn
                      )
                    ) {
                      throw new this.serverless.classes.Error(
                        this.resolveInvalidDestinationPropertyErrorMessage(functionName, 'arn')
                  ...
                  OnFailureDestinationArn = event.stream.destinationConfig.onFailure.destination.arn;
                } else if (
                  typeof event.stream.destinationConfig.onFailure.destination === 'string'
                ) {
                  if (event.stream.destinationConfig.onFailure.destination.indexOf('arn:') !== 0) {
                    throw new this.serverless.classes.Error(
                      this.resolveInvalidDestinationPropertyErrorMessage(functionName, 'arn')
                    );
                  }

                  OnFailureDestinationArn = event.stream.destinationConfig.onFailure.destination;
                } else {
                  throw new this.serverless.classes.Error(
                    this.resolveInvalidDestinationPropertyErrorMessage(functionName, 'arn')
                  );
                ...
                const destinationType =
                  event.stream.destinationConfig.onFailure.destination.type ||
